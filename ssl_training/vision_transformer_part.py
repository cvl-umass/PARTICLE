...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.projector = MLP(384, 2048, 256)
        self.mask_pool = MaskPooling(10, 6, downsample=8)
    ...
    def forward(self, x):
        ...
        x = self.norm(x)
        # B, nc, sp = x.shape
        # x[:,1:] = (x[:,1:] + self.interpolate_pos_encoding(x, 28, 28)[:,1:])*x[:,0].unsqueeze(1)
        return x[:,0], x[:,1:] # + self.interpolate_pos_encoding(x, 28, 28)[:,1:])*x[:,0].unsqueeze(1)
